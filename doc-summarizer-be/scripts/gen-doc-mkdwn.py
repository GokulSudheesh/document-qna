...
def generate_markdown(api_spec: str, title: str = None) -> str:
    api_spec = json.loads(api_spec)
    md = []
    ...
    for path, methods in paths.items():
        for method, details in methods.items():
            ...
            md.append(
                f"**Operation ID:** {details.get('operationId', 'N/A')}\n")
            ...
            if request_body:
                ...
                md.append(
                    f"- **Required:** {request_body.get('required', False)}")
                md.append("- **Content:**")
                content = request_body.get("content", {})
                for content_type, content_schema in content.items():
                    md.append(f"  - **{content_type}:**")
                    schema_ref = content_schema.get(
                        "schema", {}).get("$ref", "N/A")
                    md.append(
...
def main():
    ...
    print(f"API documentation generated successfully in {file_name}")
